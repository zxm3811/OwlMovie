...
int movielistid = Integer.parseInt(request.getParameter("id"));
MovieList movielist = mldi.findMovieListByMovieListId(movielistid);
		...
		<div id="fh5co-main">
			...
			<div>
				...
				<div class="seminar">
					...
					<!--介绍开始-->
					<div class="introduce">
						<p><%= movielist.getDescription() %></p>

					</div>
					
					<%
					int uid = movielist.getUid();
					UserDaoImpl udi2 = new UserDaoImpl();
					User user = udi2.findUserById(uid);
					%>
					
					<!--收藏开始-->
					<div class="introduce_icon_cpr"
						style="text-align: left; color: black; font-size: 16px;">
						<a href="#" style="color: lightgrey; margin-right: 10px;">创建者：<%= user.getName() %></a>
						<span><%= movielist.getCreateTime() %></span>&nbsp;&nbsp;&nbsp; <a href="#"
							style="color: lightgrey; margin-right: 10px;"> <i
							class="icon-comment"></i> <span><%= movielist.getCommentNumber() %></span>
						</a> <a href="#" style="color: lightgrey; margin-right: 10px;"> <i
							class="icon-heart"></i> <span>收藏</span>
						</a>
					...
					<div class="seminar-content">
						...
						String movieList = movielist.getMovieList();    //获取影单里的电影字符串
			...
			<div class="container-fluid">
            	...
            	<%if(listCurPage > 1) {
              	%>
                  	<a rel="nofollow" href="movielistdetails.jsp?&id=<%= movielist.getId() %>&listCurPage=<%=listCurPage-1%>" target="_self">上一页&gt; </a>
            	<%}
              	else {
              	%>
            	...
            	<%}
              	...
              	if(j <= 4) {
            	  	for(int i = 1; i <= (listPageSize>7?7:listPageSize); i++) {
            			  ...
            			  %><a rel="nofollow" href="movielistdetails.jsp?&id=<%= movielist.getId() %>&listCurPage=<%=i%>" target="_self" ><%=i %></a><%
            		  	}
            	  	}
              	}
              	else {
              	...
              	%>
                  	<a rel="nofollow" href="movielistdetails.jsp?&id=<%= movielist.getId() %>&listCurPage=<%=listCurPage+1%>" target="_self">下一页&gt; </a>
            	<%}
              	else {
              	%>
